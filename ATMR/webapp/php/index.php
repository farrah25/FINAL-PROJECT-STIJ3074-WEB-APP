...
if (isset($_GET['submit']))
{
    ...
    if ($_GET['submit'] == "cart")
    {
        if ($email != "Guest")
        {
            $id = $_GET['id'];
            $cartqty = "1"; 
            $stmt = $conn->prepare("SELECT * FROM tbl_carts WHERE user_email = '$email' AND id = '$id'");
            $stmt->execute();
            ...
            {
                $addcart = "INSERT INTO `tbl_carts`(`user_email`, `id`,`cart_qty`) VALUES ('$email', $id','$cartqty')";
                try
                {
                    $conn->exec($addcart);
        ...
        <div class="w3-grid-template">
             ...
             $cart = "cart";
                foreach ($rows as $products){
                    ...
                    <b>$name</b><br>RM $price / $cartqty avail<br>
